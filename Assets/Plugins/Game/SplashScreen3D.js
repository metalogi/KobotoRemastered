...
function Start()
{
   ...
   aud.volume = PlayerPrefs.GetFloat("MusicVolume" , 0.5);
	...
	var listener : AudioListener = Camera.main.GetComponent(AudioListener);
	listener.volume = PlayerPrefs.GetFloat("GlobalVolume");
	if (listener.volume==0) listener.pause = true;
	print ("globalVolume=" + listener.volume);
...
function Awake()
{
	 ...
	 GetComponent.<Camera>().main.farClipPlane =5;
...
logo =logo_iPad;
	...
	kubrikRect = Rect(Screen.width/2 - 0.3125 *Screen.width, Screen.height*0.075, 0.625*Screen.width, 0.36 * Screen.height);
	state=0;
}
...
function Update()
{
	...
	if ((state==4) &&(startButtonEnabled))
	{
		...
		if(gen.TouchUpdate(facebookRect))
			{
				print("facebook");
				Application.OpenURL("http://www.facebook.com/pages/Kubrik/129466013763259?v=wall");
			}
...
function SplashScreen()
{
	...
	yield FadeOut(logoFadeOut);
	 GetComponent.<Camera>().main.farClipPlane =2000;
	state=3;
	 content=splash;
...
function OnGUI()
{
	...
	switch (state)
	{
		...
		// GUI.DrawTexture(kubrikRect,splash);
		...
		//GUI.Box( FullScreenRect,  GUIContent(content), screenStyle);
		//GUI.DrawTexture(FullScreenRect, content,  ScaleMode.StretchToFill,true,  Screen.height/Screen.width);
		//if (content == splash) GUI.DrawTexture(kubrikRect, content);
		if (content == logo) GUI.DrawTexture(FullScreenRect, content);
		break;
	}
