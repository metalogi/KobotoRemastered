...
function Start()
{
	...
	if(physical){
		...
		if(!animateRotation) {ridge.constraints = RigidbodyConstraints.FreezeRotation;}
	}
...
{	
	...
	{	
		...
		if (!physical) ridge.MovePosition(updatePos);
		else ridge.AddForce((updatePos - transform.position)*ridge.mass *forceStrength*Time.fixedDeltaTime);
	...
	if (!on)
	{
		...
		if (!physical && !ridge.isKinematic) ridge.isKinematic=true;
...
{
//Debug.Log("resetin spine object");
	//ridge.MovePosition(spline.ReadSpline(startT));
	//transform.position=spline.ReadSpline(startT);
	ridge.isKinematic =!physical;
