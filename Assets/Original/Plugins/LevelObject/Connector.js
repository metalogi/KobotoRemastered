var generateOnLoad : boolean = false;
...
private var m_tang0:Vector3 ;
private var m_tang1:Vector3;
private var m_trans : Transform;

class splinePreferences
{
	var numPoints:int=6;
	var continuity:float =1;
	
}
var splinePrefs=new splinePreferences();
...
function SetParentObjs(lefty:Transform, righty:Transform)
{
	parentLeft=lefty;
	parentRight=righty;
}
...
function FindParent(side : String)
{
	var Raystart : Vector3 = Vector3.zero;
	var RayDir : Vector3 =Vector3.zero;
	
	var hit : RaycastHit;
	
	for (var i : int =0; i<4; i++) 
	{
		if (side =="left") Raystart += leftEdge[i]+ Vector3(0,0,2);
		if (side == "right") Raystart += rightEdge[i]-Vector3(0,0,2);
	}
	Raystart /=4;
	
	
	var testpoint : int;
	if (side =="left") testpoint=0;
	if (side == "right") testpoint=norms.length-1;
	RayDir.y = norms[testpoint].z;
	RayDir.z =-norms[testpoint].y;
	
	Physics.Raycast(Raystart, RayDir, hit, 100);
	if (side =="left")	parentLeft=hit.transform;
	if (side =="right")	parentRight=hit.transform;
}


function UpdateMesh(n:int)
{
	splinePrefs.numPoints =n;
...
var rightMeshFilter:MeshFilter=parentRight.GetComponent(MeshFilter);
	...
	for(var i:int=0; i<4;i++) 
	{
		...
		var iOff:Vector3;
		iOff.z =leftBB.extents.x*parentLeft.localScale.x;
		if (i<2) iOff.x =leftBB.extents.z*parentLeft.localScale.z;
		else iOff.x = -leftBB.extents.z*parentLeft.localScale.z;
		if ((i==0) || (i==2)) iOff.y=leftBB.extents.y*parentLeft.localScale.y;
		else iOff.y= -leftBB.extents.y*parentLeft.localScale.y;
		...
		iOff.z =rightBB.extents.x*parentRight.localScale.x;
		if (i<2) iOff.x =rightBB.extents.z*parentRight.localScale.z;
		else iOff.x = -rightBB.extents.z*parentRight.localScale.z;
		if ((i==0) || (i==2)) iOff.y=rightBB.extents.y*parentRight.localScale.y;
		else iOff.y= -rightBB.extents.y*parentRight.localScale.y;
...
// var m_t0 : Vector3 =(leftEdge[0] - rightEdge[0]).magnitude* splinePrefs.continuity*parentLeft.TransformDirection(Vector3(-1,0,0));
//var m_t1 : Vector3  =(leftEdge[0] - rightEdge[0]).magnitude* splinePrefs.continuity*parentRight.TransformDirection(Vector3(-1,0,0));
//var splinePoint :Vector3  = Hermite(0.5, leftEdge[0], rightEdge[0], m_t0,m_t1);

//Debug.DrawLine (leftEdge[0], splinePoint, Color.yellow);
	...
	{
	UpdateMesh();
	//debugObj.transform.position=rightEdge[Mathf.Floor(Time.time%4)];
	}
	*/
}

function GetSurfacePoint (u : float) : Vector3
{
	//if (m_tang0 == null) 
		m_tang0=(leftEdge[0] - rightEdge[0]).magnitude* splinePrefs.continuity*parentLeft.TransformDirection(Vector3(-1,0,0));
	//if (m_tang1 == null) 
	m_tang1 =(leftEdge[0] - rightEdge[0]).magnitude* splinePrefs.continuity*parentRight.TransformDirection(Vector3(-1,0,0));
	var splinePoint :Vector3  = Hermite(Mathf.Clamp01(u), leftEdge[0], rightEdge[0], m_tang0,m_tang1);
	
	
	return (Vector3(0, splinePoint.y, splinePoint.z));
}

function GetSurfaceTangent(u : float) : Vector3
{
	
	Debug.Log("SurfaceTangent u=  " + u);
	if (u > 0.95) return parentRight.TransformDirection(Vector3(1,0,0));
	if (u < 0.05) return parentLeft.TransformDirection(Vector3(1,0,0));
	
	Debug.DrawLine(GetSurfacePoint(u+0.05), GetSurfacePoint(u), Color.blue);
	Debug.Log((GetSurfacePoint(u+0.05) - GetSurfacePoint(u)).normalized);
	return (GetSurfacePoint(u+0.05) - GetSurfacePoint(u)).normalized;
}

function Hermite(t:float, p0:Vector3, p1:Vector3, m0:Vector3, m1:Vector3) : Vector3
{
...
function HermiteTangent(t:float, p0:Vector3, p1:Vector3, m0:Vector3, m1:Vector3) : Vector3
{
	var p:Vector3;
...
{ 
	var o_493_15_636265844777245078 = CMesh;
	var o_494_18_636265844777349793 = sideMesh;
}
